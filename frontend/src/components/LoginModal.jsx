// LoginModal.jsx — Email/密碼 登入 + 註冊 彈窗
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (password.length < 6) { setError('密碼至少 6 個字元'); setLoading(false); return; }
    ...
    <div style={s.backdrop} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={s.modal}>
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          {mode === 'register' && (
            <div style={s.field}>
              <label style={s.label}>顯示名稱（選填）</label>
              <input
                style={s.input}
                type="text"
                placeholder="你的名字"
        ...
        <div style={s.switchRow}>
          {mode === 'login' ? (
            <span style={s.switchText}>
              還沒有帳號？
              <button style={s.switchBtn} onClick={() => { setMode('register'); setError(''); }}>立即註冊</button>
            </span>
          ) : (
            <span style={s.switchText}>
              已有帳號？
              <button style={s.switchBtn} onClick={() => { setMode('login'); setError(''); }}>回到登入</button>
            </span>
          )}
        </div>
