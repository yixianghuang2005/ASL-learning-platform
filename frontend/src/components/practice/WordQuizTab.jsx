// WordQuizTab.jsx — 詞彙闖關：出題 → 鏡頭偵測 → 答對過關 → 答錯繼續

import React, { useState, useCallback, useEffect, useRef } from 'react';
import WordVideoCapture from '../WordVideoCapture';

const ASL_WORDS = [
  { word: 'hello',    display: 'Hello',     zh: '你好',   hint: '張開手掌，從額頭旁向外揮動' },
  { word: 'thankyou', display: 'Thank You', zh: '謝謝',   hint: '手指觸嘴，向前伸出' },
  { word: 'please',   display: 'Please',    zh: '請',     hint: '手掌貼胸口畫圓圈' },
  { word: 'sorry',    display: 'Sorry',     zh: '對不起', hint: '握拳貼胸口畫圓圈' },
  { word: 'help',     display: 'Help',      zh: '幫助',   hint: '一手拳放另一手掌，向上推' },
  { word: 'yes',      display: 'Yes',       zh: '是',     hint: '握拳上下點動' },
  { word: 'no',       display: 'No',        zh: '否',     hint: '食中指夾拇指，開合兩次' },
  { word: 'want1',    display: 'Want',      zh: '想要',   hint: '雙手彎指朝己，向內拉' },
  { word: 'like',     display: 'Like',      zh: '喜歡',   hint: '拇指中指捏胸口，向外拉開' },
  { word: 'more',     display: 'More',      zh: '更多',   hint: '雙手指尖合攏，互點兩次' },
];

const QUIZ_TOTAL = 8;

function drawQuestions(n) {
  return [...ASL_WORDS].sort(() => Math.random() - 0.5).slice(0, n);
}
...
export default function WordQuizTab() {
  ...
  const handlePass = useCallback((answeredWord, skipped = false) => {
    ...
    if (qIndex + 1 >= QUIZ_TOTAL) setPhase('result');
...
function QuizReady({ onStart }) {
  return (
    <div style={s.center}>
      ...
      <p style={s.desc}>
        系統隨機出 <strong>{QUIZ_TOTAL} 題</strong>，顯示詞彙與中文意思。<br />
        對準鏡頭比出正確手勢，系統連續辨識一致後<strong>自動過關</strong>！
      </p>
      <div style={s.rules}>
        <div style={s.rule}>📷 需要開啟攝影機</div>
        <div style={s.rule}>🎯 共 {QUIZ_TOTAL} 題，隨機出題</div>
...
function QuizPlaying({ question, qIndex, total, onPass }) {
  ...
  useEffect(() => {
    setLive(null); setFlash(null); setShowHint(false);
...
const s = {
  ...
  questionWord:  { fontSize: 44, fontWeight: 900, color: '#3b82f6', lineHeight: 1 },
  questionZh:    { fontSize: 22, fontWeight: 600, color: '#f1f5f9' },
